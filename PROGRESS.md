...
### سیستمِ بِنتوی قابِ ثابت — پایلوتِ داشبورد (۲۰۲۶-۰۶-۱۵) — DECISION-099
- نقدِ ریشه‌ای: سایزِ تایل به محتوا وابسته بود → آنارشیِ ساختاری. بنچ‌مارک (Apple/Datadog): «اندازه از اهمیت، نه حجمِ محتوا».
- داشبورد به سیستمِ **قابِ ثابت** (گرید ۱۲ستونه + `grid-auto-rows:206px` + footprintِ N×M + clamp) پیاده شد. تستِ سه‌حالته: تایلِ هدف خالی/پُر **هم‌اندازه** → چیدمان پایدار، منطبق با `mockups/bento-system.html`.
- پایپ‌لاینِ وفاداری: قابِ ثابت + پورتِ مو‌به‌مو + **گیتِ اسکرین‌شات**. قدمِ بعد: همین سیستم روی پروفایل و هدف. ایزولاسیونِ CSS Modules موکول به محیطِ سالم.

### تثبیتِ بِنتو — جداسازیِ ستون‌ها تا تایل‌ها همدیگر را نکشند (۲۰۲۶-۰۶-۱۵) — DECISION-098
- باگ: از زمانِ ریدیزاینِ بِنتو، تغییرِ محتوای یک تایل (مثلاً ثبتِ هدف/استوری) تایل‌های همسایه را می‌کشید و چیدمانِ داشبورد/پروفایل/هدف را بهم می‌زد.
- ریشه: `grid-template-areas` + spanِ دوردیفه + `align-items:stretch` = کوپلینگِ ارتفاعی. رفع: ستون‌های `flex`-ِ مستقل (`.dsh-col`/`.pf-col`) + `align-items:start` در هر سه بِنتو. تستِ بصریِ سه‌حالته تأیید کرد ستونِ همسایه دیگر کشیده نمی‌شود.

### یکپارچگیِ عرضِ کانتینرِ صفحات با navbar (۲۰۲۶-۰۶-۱۵) — DECISION-097
- صفحهٔ `/plans` گاترِ `px-5` داشت و ۴px از navbar بیرون می‌زد → به `px-6` اصلاح شد. حالا داشبورد/پروفایل/هدف/پلن همگی ۱۰۲۴px/۲۴px = منطبق با navbar. صفحاتِ فرم/خواندنیِ باریک عمداً وسط‌چین.
- **حادثهٔ بازیابی:** کرشِ سیستم/ادیتور تحتِ فشارِ RAM، `globals.css` + `DECISIONS.md` + `PROGRESS.md` را به نسخهٔ قدیمی برگرداند (CSS از `.next` و docs از خلاصه/TASKS بازسازی شد). جزئیات: [[editor-revert-and-memory-pressure]].

### بازطراحیِ پروفایل کاربر — بِنتو + کیف‌پولِ هیرو + دراورِ پشتیبانی (۲۰۲۶-۰۶-۱۵) — DECISION-096
- `/settings/profile` به بِنتوِ تایل‌بندی (مثل داشبورد) بازطراحی شد؛ پورتِ `pf-` از `mockups/profile-redesign.html`.
- هیرو دست‌نخورده + کیف‌پولِ فلت؛ نوارِ آمار حذف؛ تایلِ مالی (شارژ+کارت+مودالِ تراکنش‌ها)؛ دراورِ پشتیبانیِ inline؛ تایل‌های پلن و یادآوری (فقط UI). `/wallet`/`/support` دست‌نخورده.

### یکپارچه‌سازیِ داشبورد/هدف — ۳ فاز (۲۰۲۶-۰۶-۱۵) — DECISION-092/093/094/095
- داشبوردِ «امروز + مسیرِ من» + حذفِ /history؛ هدف/چالش (`Goal.type`) + سقفِ ۳۰ روز + `JourneyRail`؛ کتابخانهٔ مسیرها + بازخوانیِ سفر. بازپیاده‌سازیِ مو‌به‌مو طبقِ ماکاپ (پورتِ `dsh-`/`jp-`).
