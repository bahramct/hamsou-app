...
//   POST : تولیدِ بینشِ امروز — Pro، روزِ ۲ تا قبل از پایان، یک‌بار در روز
...
export async function POST(request: NextRequest, { params }: Ctx) {
  ...
  // ── پنجرهٔ زمانی: روزِ دوم تا قبل از پایان ─────────────────────────────────
  const win = companionWindow(goal.startDate, goal.endDate);
  if (!win.available) {
    const msg =
      win.reason === "before_day_2"
        ? "راهنماییِ «همراه» از روزِ دومِ مسیر در دسترس است."
