...
> **یادداشتِ بازیابی (۲۰۲۶-۰۶-۱۵):** DECISIONهای ۰۹۰–۰۹۶ در یک کرشِ سیستم/ادیتور از این فایل پاک شدند (هرگز commit نشده بودند، پس git نسخه نداشت). ۰۹۵/۰۹۶ کامل و ۰۹۰–۰۹۴ از TASKS.md و تاریخچهٔ commit بازسازی شدند — کد همگی commit‌شده و سالم است. [[editor-revert-and-memory-pressure]]

### DECISION-101 | سیستمِ چیدمانِ نهایی — «محتوا-تطبیقی» (نه یک قالبِ سفتِ واحد)
- **تاریخ:** ۲۰۲۶-۰۶-۱۵ · **وضعیت:** ✅ تأیید شده (تستِ بصری)
- **زمینه:** بعد از پایلوتِ قابِ ثابتِ داشبورد (DECISION-099)، سؤال این بود که آیا همان را روی پروفایل و هدف هم اعمال کنیم.
- **یافتهٔ تخصصی:** قابِ ثابت برای صفحاتِ **نگاه‌سریع (glanceable)** عالی است، اما پروفایل و هدف **تعاملی‌اند**: پروفایل ویرایشگرِ inline (هویت/اطلاعات شخصی) و هدف آهنگ‌سازِ استوری (textarea) دارد که باید **باز/رشد** شوند. قابِ ثابت با `overflow:hidden` این‌ها را کلیپ می‌کرد.
- **تصمیم (انتخابِ مالک):** سیستمِ واحد = **یک عرض (navbar)، یک گاتر (۱۶px)، یک زبانِ بصری، تایل‌های مستقلِ بدونِ کوپلینگ** در هر سه صفحه؛ تنها **سفتیِ قاب** بر اساسِ نوعِ محتوا فرق می‌کند:
  - **داشبورد** (نگاه‌سریع) → **قابِ ثابتِ** ۱۲ستونه (DECISION-099).
  - **پروفایل/هدف** (تعاملی) → **ستون‌های مستقلِ منعطف** (DECISION-098) — تایل به محتوا سایز می‌گیرد ولی ستون‌ها decoupled‌اند، پس کوپلینگ/بهم‌ریختگی نیست و ویرایش نمی‌شکند.
- **نتیجه:** هر سه صفحه هم‌راستا با navbar (DECISION-100) + پایدار + یکدست. «آنارشی»ی ساختاری در کلِ سه صفحه حل شد؛ پروفایل/هدف بدونِ تغییرِ کد آماده‌اند (فقط رفعِ نکته ۱ روی wrapها کافی بود).

### DECISION-100 | پنج اصلاحِ داشبورد: هم‌ترازیِ عرض با navbar + Portalها + لینک‌ها + پاپ‌اوورها
- **تاریخ:** ۲۰۲۶-۰۶-۱۵ · **وضعیت:** ✅ (تستِ measure + اسکرین)
- **#۱ هم‌ترازیِ عرض با navbar:** محتوای صفحه ~۱۰۰px از navbar باریک‌تر بود. ریشه (با measure): wrap داخلِ flex-columnِ AppShell با `margin:0 auto` به **عرضِ محتوا** جمع می‌شد و `width:100%` هم جواب نداد (رفتارِ auto-marginِ فلکس). راهِ مطمئن: `width:min(1024px,100%) + margin-inline:0 + align-self:center` روی `.dsh-wrap/.pf-wrap/.jp-wrap`. تأیید: هر سه صفحه دقیقاً `l=164,r=1188,w=1024` = navbar.
- **#۲ مودالِ تاریخچهٔ اخیر:** از الگوی مودال پیروی نمی‌کرد → `createPortal` به body (وسط‌چین + backdrop).
- **#۳ لینک‌های داشبورد:** `/wallet`→`/settings/profile#finance` و `/support`→`/settings/profile?support=1` (دراور خودکار باز می‌شود؛ SupportCenter پارامتر را می‌خواند). صفحاتِ قدیمی دیگر باز نمی‌شوند.
- **#۴ حبابِ تقویمِ هفتگی:** `jp-tip` با `position:fixed` داخلِ `.dsh-tp`ی `backdrop-filter`دار، containing-block اشتباه می‌گرفت → `createPortal` به body تا مختصاتِ viewport درست شود (بالای سلول).
- **#۵ پاپ‌اوورِ تایم‌لاینِ هدف:** `overflow:hidden`ِ قابِ ثابت (DECISION-099) پاپ‌اوور را کلیپ می‌کرد → `.t-goal { overflow: visible }` + clampِ عنوان تا محتوا همچنان در قاب جا شود.
- **بدونِ اثرِ مخرب:** فقط داشبورد + سه wrap؛ پروفایل/هدف عرضشان هم درست شد (همان سه کلاس). `eslint` ✅.

### DECISION-099 | سیستمِ بِنتوی قابِ ثابت (داشبورد — پایلوت) + پایپ‌لاینِ وفاداری به ماکاپ
- **تاریخ:** ۲۰۲۶-۰۶-۱۵
- **وضعیت:** ✅ داشبورد پیاده‌شده (پایلوت، تستِ بصریِ سه‌حالته)
- **زمینه:** نقدِ استراتژیکِ مالک: بِنتو «آنارشیِ» ساختاری دارد چون **سایزِ تایل به محتوا وابسته است** و تایل‌های هم‌ردیف روی هم اثر می‌گذارند و هر صفحه عرض/چیدمانِ متفاوت دارد. هم‌چنین ایرادِ متا: ماکاپِ تأییدشده مو‌به‌مو پیاده نمی‌شد.
- **بنچ‌مارک:** بزرگ‌ترین اشتباهِ بِنتو طبقِ منابع (Apple/Google/Datadog، orbix، studiomeyer) = «اندازهٔ تایل از حجمِ محتوا بیاید نه از اهمیت». راه‌حل = `grid-auto-rows` با واحدِ ثابت + footprintِ N×M + محتوا داخل (clamp/scroll).
- **تصمیم — سیستمِ قابِ ثابت (انتخابِ مالک از بینِ ۳ گزینه):**
  - گریدِ واحد: `grid-template-columns: repeat(12,1fr)`، `grid-auto-rows: 206px`، گاترِ ۱۶px، هم‌عرضِ navbar (۱۰۲۴px).
  - footprintِ ثابت: هدف `۷×۲`، نبض/تاریخچه `۵×۱`، گزارش `۷×۱`، پلن `۵×۱`. تایل `overflow:hidden`؛ متن‌ها `line-clamp`.
  - **نتیجه (تستِ بصری):** تایلِ هدف چه خالی چه پُر **هم‌اندازه** می‌ماند → کوپلینگِ ارتفاعی صفر، چیدمان پایدار، منطبق با ماکاپ `mockups/bento-system.html`.
- **پایپ‌لاینِ وفاداری (راهکارِ ایرادِ متا «پیاده‌سازی ≠ ماکاپ»):** (۱) قابِ ثابت تا محتوا چیدمان را خراب نکند؛ (۲) پورتِ مو‌به‌مو؛ (۳) **گیتِ اسکرین‌شات** — بعد از هر صفحه با هارنس اسکرین می‌گیرم و کنارِ ماکاپ می‌گذارم تا مو‌به‌مو شود. ریشهٔ دیگرِ واگرایی = **revertِ ادیتور تحتِ فشارِ RAM** که globals.css را پاک می‌کرد → کاهش با commitِ فوری؛ ایزولاسیونِ کاملِ CSS Modules به‌عنوان پاسِ سخت‌سازیِ بعدی موکول شد (محیطِ سالم لازم دارد). [[editor-revert-and-memory-pressure]]
- **بدونِ اثرِ مخرب:** فقط بخشِ داشبوردِ بلوکِ بِنتو در `globals.css` (پیشوندِ `dsh-`، ایزولهٔ عملی) + حذفِ wrapperِ ستون در page؛ پروفایل/هدف دست‌نخورده (هنوز DECISION-098). `eslint` ✅. **قدمِ بعد:** اعمالِ همین سیستم روی پروفایل و هدف.

### DECISION-098 | تثبیتِ بِنتو — جداسازیِ ستون‌ها تا تایل‌ها همدیگر را نکشند
- **تاریخ:** ۲۰۲۶-۰۶-۱۵
- **وضعیت:** ✅ تأیید شده (تستِ بصریِ سه‌حالته)
- **زمینه:** صاحب پروژه دید که از زمانِ ریدیزاینِ بِنتو، چیدمانِ صفحات وابسته به محتوا **ناپایدار** است: تا کاربر هدف/چالش ثبت می‌کند یا استوری می‌نویسد، تایلِ هدف بلند می‌شود و **تایل‌های همسایه را می‌کشد** و کلِ داشبورد/برنامه‌ریزی بهم می‌ریزد.
- **ریشه:** بِنتوها `display:grid` با `grid-template-areas` بودند و تایلِ بزرگ **دو ردیف را span** می‌کرد. در CSS Grid، ارتفاعِ ردیف از محتوا می‌آید و پیش‌فرضِ `align-items:stretch` تایل‌های هم‌ردیف را تا ارتفاعِ ردیفِ بزرگ‌شده می‌کشد → کوپلینگِ ارتفاعیِ بینِ تایل‌های مستقل.
- **تصمیم — جداسازیِ ستون‌ها (تستِ بصری با ۳ حالت: بدون‌هدف/با‌هدف/با‌استوری):**
  - **داشبورد (`.dsh-bento`)** و **پروفایل (`.pf-bento`)**: هر ستون = `flex`- ستونیِ **مستقل** (`.dsh-col` / `.pf-bento-cols .pf-col`)؛ `grid-template-areas` حذف و `align-items:start`. ارتفاعِ هر تایل فقط به محتوای خودش وابسته است؛ تایلِ هدف/مالی که بلند شود فقط ستونِ خودش را بلند می‌کند، نه همسایه را. پشتیبانیِ پروفایل تمام‌عرضِ زیرِ ستون‌ها.
  - **هدف (`.jp-bento`)**: فقط `align-items:start` (چون span چندردیفه ندارد) → تایلِ «همراه» دیگر تا ارتفاعِ آهنگ‌سازِ استوری کشیده نمی‌شود.
  - تخصیصِ ستون‌ها همان ماکاپ است (هدف+گزارش | نبض+تاریخچه+پلن)؛ فقط کفِ ستون‌ها آزاد (ragged) است نه کوپل‌شده.
- **بدونِ اثرِ مخرب:** فقط wrapperِ ستون در JSX + override در `globals.css` (بعد از بلوکِ بِنتو، بدونِ دست‌زدن به خودِ تایل‌ها)؛ `t-*`/`pf-t-*` در flex بی‌اثر و بی‌ضرر. `eslint` ✅؛ تستِ بصریِ سه‌حالته نشان داد ستونِ همسایه دیگر کشیده نمی‌شود.

### DECISION-097 | یکپارچگیِ عرضِ کانتینرِ صفحات با navbar
- **تاریخ:** ۲۰۲۶-۰۶-۱۵
- **وضعیت:** ✅ تأیید شده
- **زمینه:** صاحب پروژه دید عرضِ محتوای صفحات با هم یکی نیست (با ملاکِ navbar). **navbar نباید تغییر کند.**
- **ریشه:** navbar = `max-w-5xl px-6` (۱۰۲۴px، گاترِ ۲۴px). داشبورد (`.dsh-wrap`)، پروفایل (`.pf-wrap`)، هدف (`.jp-wrap`) همگی ۱۰۲۴px/۲۴px = منطبق. اما `/plans` گاترِ `px-5` (۲۰px) داشت → محتوا ۴px از هر طرف بیرونِ navbar می‌زد.
- **تصمیم:** `/plans` → `px-6`. **قانون:** هر صفحهٔ wide/تایل‌بندیِ authenticated باید همان عرض/گاترِ navbar (۱۰۲۴px / ۲۴px) را داشته باشد. صفحاتِ فرم/خواندنیِ باریک (`login`, `reports/weekly`, `support`, `wallet`) عمداً وسط‌چین و باریک می‌مانند (تأییدِ مالک: «فرمِ تک طبیعی است وسط باشد»). فرمِ ساختِ هدف هم `mx-auto max-w-lg` (وسط) و با هدفِ فعال `jp-bento` کلِ ۱۰۲۴px را پر می‌کند.
- **هم‌ترازی/بدونِ اثرِ مخرب:** فقط گاترِ صفحهٔ سایت؛ پنلِ ادمین (AdminShell) کانتینرِ مستقل دارد و دست‌نخورده.

---

### DECISION-096 | بازطراحیِ پروفایل کاربر — بِنتوِ تایل‌بندی + کیف‌پولِ هیرو + دراورِ پشتیبانی
- **تاریخ:** 2026-06-15
- **وضعیت:** ✅ تأیید شده
- **زمینه:** بعد از داشبورد و «برنامه‌ریزی و چالش»، `/settings/profile` به سبکِ بِنتوِ مدرن بازطراحی شد. بنچ‌مارک (fintech bento، الگوی کیف‌پول، Intercom درای‌پنلِ پشتیبانی)؛ ماکاپِ `mockups/profile-redesign.html` تأیید شد (با دو اصلاحِ مالک).
- **تصمیم — منبعِ حقیقت = ماکاپ؛ پورتِ CSS با پیشوندِ `pf-` (روشِ DECISION-095):**
  - **هیرو دست‌نخورده** + `.pf-wallet-mini` (آیکونِ فلتِ وکتوریِ بی‌رنگ + موجودی) در گوشهٔ خالی؛ **بدونِ پاپ‌اوور** (اصلاحِ مالک: پیلِ رنگی رد شد). **نوارِ آمار حذف شد.**
  - بِنتو: `"identity finance"/"personal finance"/"plan reminders"/"support support"`.
  - **امور مالی:** نوارِ موجودی + شارژ (`TopupPanel` بازاستفاده) + کارت‌به‌کارت + مودالِ تراکنش‌ها با فیلتر. **پروسهٔ شارژ به اینجا منتقل شد.**
  - **پشتیبانی:** دراورِ کناریِ inline (تیکتِ تازه + فهرست + گفتگو)؛ GETهای افزایشیِ `tickets` و `tickets/[id]/messages`. `/wallet` و `/support` دست‌نخورده.
  - تایلِ «یادآوری‌ها» (**فقط UI**، بدون migration)؛ تایلِ «پلن». `IdentityCard`/`PersonalInfoSection` ری‌اسکین به `.pf-field`.
- **بدونِ اثرِ مخرب:** بازگشتِ درگاه + دیپ‌لینکِ اعلان‌ها سالم؛ گیتِ `support.ticketing` و شارژِ ادمین بدون تغییر. `tsc`/lint ✅؛ تأییدِ بصریِ زنده با `scripts/shoot-profile-live.mjs`.

---

### DECISION-095 | بازپیاده‌سازیِ مو‌به‌موی داشبورد و «برنامه‌ریزی و چالش» طبقِ ماکاپ‌ها
- **تاریخ:** 2026-06-15
- **وضعیت:** ✅ تأیید شده
- **زمینه:** نسخهٔ اولِ فازهای ۱–۳ از نظرِ عملکرد درست بود اما از نظرِ ظاهر/چیدمان به ماکاپ‌های `mockups/` وفادار نبود (بازخوردِ مالک). باید **دقیقاً** پیاده شود.
- **تصمیم — منبعِ حقیقت = فایل‌های ماکاپ:** پورتِ CSS به `globals.css` با ترجمهٔ توکن (`--sage`→`--color-sage`، `--line`→`rgba(var(--rgb-line),.06)`، `--glass-strong-bg`→`rgba(var(--rgb-card),.72)`) و پیشوندِ `dsh-`/`jp-`. داشبورد: top-grid «تعهد امروز | کادر سبز» + بِنتوی CSS-Grid که تایلِ هدف دو ردیف را می‌گیرد (تایم‌لاینِ ریز + استوری + همراه). برنامه‌ریزی: بِنتوی `"hero comp"/"path path"` + ریل + «مسیرهای گذشته». واریانتِ `bare` به `StoryComposer`/`CompanionPanel`. هیروِ گیت‌دار دست‌نخورده.
- **بدونِ اثرِ مخرب:** کلاس‌ها پیشونددار/ایزوله؛ `.glass`/توکن‌ها بازاستفاده؛ override برای تینتِ طلاییِ `.jp-comp` در دارک.

---

### DECISION-105 | بازطراحیِ JourneyRail — پیکان‌های زنجیره‌ای RTL
- **تاریخ:** 2026-06-17 · **وضعیت:** ✅ پیاده‌سازی‌شده
- **زمینه:** تایم‌لاینِ قدیمی (نقطه‌های دایره‌ای + خطِ افقی) از نظرِ بصری ضعیف بود؛ درخواستِ مالک: طراحیِ پیکان‌های زنجیره‌ای مطابقِ تصویرِ مرجع.
- **تصمیمات:**
  - **جهت:** RTL بومی — نوکِ پیکان به سمتِ چپ (← همراستا با خواندنِ فارسی)؛ روزِ ۱ در راست، روزِ آخر در چپ.
  - **شکل:** `clip-path: polygon(0% 50%, 14px 0%, 100% 0%, 100% 100%, 14px 100%)` — پیکانِ ساده بدونِ notch؛ عمقِ نوک ۱۴px، عرضِ پیکان ۸۸px.
  - **زنجیره:** چیدمانِ absolute با `left = (len-1-index)*74 + 8`؛ `z-index = len - index` تا روزِ ۱ بالاترین لایه.
  - **حبابِ استوری:** فقط روزهایی که محتوا دارند (مینیمالیسم). حبابِ ثابت ۸۰×۸۰px پایینِ پیکان.
  - **نشانِ همراه:** دایرهٔ طلایی ۲۲px بالای پیکان (فقط اگر `insight` وجود داشته باشد).
  - **اسکرول:** `direction: ltr` روی `.jp-rail-vp` برای `scrollLeft` منظم؛ `direction: rtl` روی `.jp-chev-track` برای متنِ فارسی.
  - **حذف‌شده:** Portal/tooltip، `jp-line`، `jp-line-done`، `jp-pnode`، `jp-pdot`، `jp-cm`، `jp-cm-empty`.
- **بدونِ اثرِ مخرب:** `GoalTile` (`.dsh-tl`) دست‌نخورده؛ CSS با پیشوندِ `jp-cv-` / `jp-chev-` ایزوله.

---

### DECISION-094 | یکپارچه‌سازی داشبورد — فاز ۳: «کتابخانهٔ مسیرها» + «بازخوانیِ سفر»
- **تاریخ:** 2026-06-15 · **وضعیت:** ✅
- بازنویسیِ `/goal/history` + بخشِ «مسیرهای گذشته» در پایینِ `/goal`: کارت‌های پرروح (بَجِ نوع، کاورِ گرادیانی، بازه، شمارشِ استوری/بینش، جوهر). مودالِ «بازخوانیِ سفر» (`JourneyRecapModal` + `GET /api/goal/[id]/recap`) با نخِ روزها (استوری + بینشِ همراه). جمع‌بندیِ AIِ `journey-recap` = کارِ آینده. (بدنه پس از revert از TASKS.md بازسازی شد.)

### DECISION-093 | یکپارچه‌سازی داشبورد — فاز ۲: هدف/چالش + سقفِ ۳۰ روز + ریلِ سفر
- **تاریخ:** 2026-06-15 · **وضعیت:** ✅
- افزودنِ `Goal.type` (`goal|challenge`، `db push`، تأییدشده). سلکتورِ سگمنتیِ نوع + `GoalTypeBadge` (هدف=sage، چالش=ember). `MAX_RANGE_DAYS` از ۳۶۵ به **۳۰** (toast در ساخت+ویرایش). `JourneyRail` جایگزینِ `DayCard` (ریلِ افقیِ تخت + دو فلش + tooltip + نشانِ همراه). navbar «برنامه‌ریزی» → «برنامه‌ریزی و چالش». پریتی: گیتِ `goal.companion` برای هر دو نوع یکسان. (بدنه پس از revert از TASKS.md بازسازی شد.)

### DECISION-092 | یکپارچه‌سازی داشبورد — فاز ۱: «امروز + مسیرِ من» + حذفِ صفحهٔ تاریخچه
- **تاریخ:** 2026-06-15 · **وضعیت:** ✅
- بازنویسیِ `/dashboard`: ردیفِ بالا «کادر سبزِ TodayPanel | هیروِ گیت‌دارِ تعهد» + سرتیترِ «مسیرِ من» + بِنتو (`GoalTile`/`PulseTile`/`RecentTile`/`ReportTile`/`PlanTile`). helperِ `lib/dashboard/activity.ts`. حذفِ کاملِ `/history` (صفحه + API + `HistoryList`/`HistoryItem`/`types/history`) → مودالِ ۳۰‌روزه (`RecentHistoryModal` controlled) و حذفِ آیتمِ navbar. (بدنه پس از revert از TASKS.md بازسازی شد.)

### DECISION-091 | Portal برای GoalEditModal/GoalDeleteModal + FAB گرم + حذفِ SupportSection از پروفایل
- **تاریخ:** ۲۰۲۶-۰۶-۱۴ · **وضعیت:** ✅
- مودال‌های ویرایش/حذفِ هدف به الگوی Portal ([[modal_design_pattern]]) منتقل شدند؛ FABِ چت با تنِ گرم؛ کارتِ چتِ آنلاین از پروفایل حذف و دسترسی از FABِ جهانی شد. (بدنهٔ کامل در revert گم شد؛ هدر از تاریخچه.)

### DECISION-090 | بستهٔ رفعِ باگ‌های UX
- **تاریخ:** ۲۰۲۶-۰۶-۱۴ · **وضعیت:** ✅
- مودال‌ها به Portal، رفعِ چتِ FAB دوگانه، ارقامِ فارسی، placeholderها و اصلاحاتِ navbar. (بدنهٔ کامل در revert گم شد؛ هدر از تاریخچه.)

---

### DECISION-089 | سازندهٔ کاملِ onboarding در پنل + حذفِ اسلایدِ همدم + شارژِ دستیِ کیف‌پول
- **تاریخ:** ۲۰۲۶-۰۶-۱۴
- **وضعیت:** ✅ تأیید شده (پیاده‌سازی‌شده)
- **زمینه:** سه درخواستِ صاحب پروژه: (۱) حذفِ اسلایدِ تغییرِ نامِ همدم (کاربر مجاز به تغییرِ آن نیست)؛
  (۲) مدیریتِ کاملِ onboarding از پنل (افزودن/حذفِ اسلاید + ویرایشِ همهٔ متن‌ها)؛ (۳) شارژِ دستیِ کیف‌پولِ
  کاربر از صفحهٔ جزئیاتِ کاربر در پنل. تصمیم‌ها با پرسشِ ویژوال: «سازندهٔ کامل» + «شارژ + کسر».
- **(۱) حذفِ اسلایدِ همدم + قفلِ نامِ همدم (admin-controlled):**
  - اسلایدِ همدم از پیش‌فرضِ onboarding حذف شد.
  - `companionName` از `POST /api/onboarding/complete` و از `PATCH /api/profile` برداشته شد
    (کاربر دیگر هیچ‌جا نمی‌تواند نامِ همدم را تغییر دهد).
  - **رفعِ ناهماهنگی:** `layout.tsx` وقتی `companionName` کاربر null بود به «همدم»ِ هاردکد برمی‌گشت،
    در حالی که `api/chat/messages` به پیش‌فرضِ ادمین (`chat.companion.defaultName`) برمی‌گردد. layout اصلاح شد
    تا همان پیش‌فرضِ ادمین را بخواند → هدرِ FAB/ChatWindow با پیامِ خوش‌آمدِ چت یکی شد (هم‌ترازی).
- **(۲) سازندهٔ کاملِ onboarding (CMS سبک):**
  - اسلایدها در `AppSetting` کلیدِ `onboarding.config` به‌صورتِ JSON (بدونِ migration). منبعِ حقیقتِ شکل +
    پیش‌فرض + نرمال‌سازیِ امن در `src/lib/onboarding/config.ts` (Zod). انواع: `narrative` (نامحدود، add/remove/
    reorder)، `name`/`motive` (کارکردی، حداکثر یکی، قابلِ خاموش‌کردن، متن‌شان ویرایش‌پذیر)، `final`
    (همیشه یکی، همیشه آخر، حذف‌ناپذیر). placeholderِ `{name}` در متن → نامِ کاربر.
  - گزینه‌های انگیزه: slug ثابت (داده پایدار)، فقط label ویرایش‌پذیر.
  - **رندرر:** `OnboardingFlow` کاملاً از config رندر می‌کند (هیچ اسلایدِ هاردکدی نمانده).
  - **پنل:** بخشِ «تنظیمات سایت» → `OnboardingBuilder` (افزودن/حذف/جابجایی + ویرایشِ همهٔ متن‌ها) +
    `GET/POST /api/admin/settings` (اعتبارسنجیِ Zod + نرمال‌سازی سمتِ سرور). کلاینت فقط `import type` از
    config می‌کند تا prisma واردِ بسته نشود.
  - **هم‌ترازی:** برچسبِ «انگیزهٔ ورود» در صفحهٔ کاربرِ پنل از همان configِ زنده resolve می‌شود
    (`motiveLabelFromConfig`).
- **(۳) شارژِ دستیِ کیف‌پول:**
  - تابعِ `adjustBalance` (از قبل موجود، اتمیک، +/−، گاردِ منفی‌نشدن) به UI وصل شد.
  - `POST /api/admin/users/[id]/wallet` (enforce `payment.manage`) + `AdminWalletCharge` (شارژ/کسر، مبلغِ
    تومان با ارقامِ فارسی، یادداشتِ اجباری) + اعلانِ تازهٔ `wallet.adjusted` به کاربر + audit log.
- **تست:** `tsc --noEmit` ✅ · `next build` ✅. بدونِ اثرِ مخرب — همه افزایشی، مسیرهای موجود دست‌نخورده.

---

### DECISION-088 | ریدیزاینِ Notion برای onboarding + toggle ادمین + پرسشِ شخصی‌ساز + تشخیصِ کندیِ Turbopack
- **تاریخ:** ۲۰۲۶-۰۶-۱۴
- **وضعیت:** ✅ تأیید شده (پیاده‌سازی‌شده)
- **زمینه:** صاحب پروژه گزارش داد سفرِ onboarding (DECISION-086) اپ را شدیداً کند کرده (هر صفحه > ۳۰ ثانیه)
  و خواست (۱) ریدیزاین به سبکِ Notion، (۲) قابلیتِ روشن/خاموش از پنلِ ادمین، و در صورتِ ماندنِ کندی، حذفِ فیچر.
- **تشخیصِ کندی (مهم):** ردیابیِ کامل نشان داد **هیچ گلوگاهی در کدِ onboarding نیست** — `proxy.ts` فقط
  JWT را verify می‌کند (بدونِ DB)، `layout.tsx` یک ستونِ ساده اضافه کرده، `getAiConfig`/`AmbientField` سبک‌اند.
  علتِ واقعی **باگِ شناخته‌شدهٔ Turbopack در Next 16** است (issueهای رسمیِ vercel/next.js #80357، #85744):
  «اولین کامپایلِ هر صفحه در dev، ۴–۵ برابر کندتر؛ ۳۰ ثانیه تا چند دقیقه». محرّک در اینجا: **`db push` وسطِ
  اجرای dev** → بازسازیِ Prisma client → invalidate شدنِ کلِ گرافِ Turbopack + بزرگ‌شدنِ کشِ `.next`.
  **در production (`next build && next start`) این پدیده وجود ندارد.** رفعِ ریشه: پاک‌کردنِ `.next` + ری‌استارتِ dev.
  → حذفِ onboarding کندیِ dev را حل نمی‌کرد؛ پس فیچر حفظ و سبک‌تر شد.
- **تصمیم‌های نهایی (پرسشِ ویژوال):** مسیر = «سبک‌سازیِ Notion + toggle + رفعِ ریشه»؛ جای toggle =
  بخشِ تازهٔ «تنظیمات سایت»؛ طراحی = «روایت + یک پرسشِ شخصی‌ساز».
- **پیاده‌سازی:**
  - **ریدیزاینِ Notion:** پس‌زمینهٔ تمیز (حذفِ `AmbientField` از `onboarding/page.tsx` + `loading.tsx`)،
    پنج پردهٔ تک‌تمرکز، کپیِ موجز. `OnboardingFlow` بازنویسی شد.
  - **پرسشِ شخصی‌ساز (امضای Notion):** پردهٔ «چه چیزی تو را به همسو آورد؟» با ۴ گزینهٔ اختیاری.
    کاتالوگِ پایدار `src/lib/onboarding/motives.ts` (slug + برچسب). فیلدِ `User.onboardingMotive String?`
    (`db push` ✅). استفاده: لحنِ پردهٔ پایانی + نمایش در پنلِ ادمین (`/admin/users/[id]` → «انگیزهٔ ورود»).
  - **toggle ادمین (هم‌ترازی):** کلیدِ `onboarding.enabled` در `AppSetting` (پیش‌فرض روشن)؛ resolverِ
    `isOnboardingEnabled()` در `src/lib/settings/site.ts`. بخشِ تازهٔ `/admin/settings` («تنظیمات سایت»)
    + `/api/admin/settings` + کامپوننتِ `SiteSettings`. permissionهای تازهٔ `settings.read`/`settings.manage`
    (گروهِ `settings`) در کاتالوگ + `npm run seed` برای sync با نقش‌ها (owner=۳۰، admin=۲۸ دسترسی).
  - **گِیتِ هم‌تراز:** هر سه نقطهٔ ورود به سفر toggle را چک می‌کنند — `onboarding/page.tsx`،
    `verify-otp` (`isNew = onboardedAt===null && isOnboardingEnabled()`)، ریدایرکتِ `settings/profile`.
  - **آیکونِ `gear`** به `AdminShell` افزوده شد.
- **هم‌ترازی:** خاموش‌کردنِ toggle از پنل → بلافاصله (cache TTL ۱۰s) کاربرِ جدید مستقیم به داشبورد می‌رود؛
  انگیزهٔ ورودِ کاربر در صفحهٔ کاربرِ پنل دیده می‌شود. هیچ اثرِ مخرب روی مسیرهای موجود (فقط افزایشی).
- **مانیفست:** پرسش دربارهٔ «انگیزه» است نه «هدف/تسک» و کاملاً اختیاری → گاردِ ضدِ Task Manager حفظ شد.
...
## DECISION-102 — رفعِ ایراداتِ پروفایل/داشبورد/سراسری (دستهٔ اول)

**تاریخ:** ۲۰۲۶-۰۶-۱۶ — **وضعیت:** اعمال‌شده

دستهٔ اولِ اصلاحاتِ بازخوردِ مالک (از ۱۲ مورد، ۸ موردِ کم‌ریسک):

- **#7 کیف‌پولِ هیرو:** برچسب «کیف‌پول» → «کیف پول شما» (چیدمان: آیکون + برچسب + عدد + «تومان»).
- **#8 کارتِ اطلاعات شخصی:** بازنویسی به ویرایشِ inlineِ ردیف‌به‌ردیف، هم‌شکلِ کارتِ «هویت و ورود» (همان input/دکمه). «دربارهٔ من» هم ویرایش‌پذیرِ inline شد. تاریخ تولد جلالیِ «۲۰ مرداد ۱۳۶۱».
- **#9 حذفِ کارتِ «پلن و اشتراک» از پروفایل:** اطلاعاتش در هیرو هست (PlanTile + import حذف شد).
- **#6 لینکِ پروفایل در navbar:** حذفِ حالتِ «انتخاب‌ماندنِ» چسبان (bg-black/6) → اکشنِ خنثی مثل «خروج» (دسکتاپ + موبایل).
- **#10 دراورِ پشتیبانی:** از شیشه‌ای به سطحِ ماتِ `rgb(var(--rgb-card))` (حذفِ backdrop-filter).
- **#11 رسیدِ تراکنش:** z-index از ۵۰ به `z-[100]` تا بالای مودالِ «همهٔ تراکنش‌ها» (`.pf-modal` z-90) بنشیند.
- **#12 مودالِ «همهٔ تراکنش‌ها»:** حذفِ `glass-strong` + پس‌زمینهٔ ماتِ `rgb(var(--rgb-card))`.
- **#4 بدونِ انتخاب/کپیِ متن در کلِ سایت:** `user-select:none` روی `body`؛ استثنا = ورودی‌ها و کلِ پنلِ ادمین (`.admin-selectable`، چون ادمین باید refCode/شماره را کپی کند). autocomplete از قبل با `<DisableAutofill>` سراسری خنثی است.

موارد باقی‌مانده (دستهٔ دوم، نیازمندِ بررسیِ زنده/تصمیم): #1 دراورِ پشتیبانی در خودِ داشبورد · #2 خطای ثبتِ استوری · #3 ترازِ لینکِ کارتِ گزارش · #5 یکدست‌سازیِ آیکون‌های سایت.

### DECISION-102 (دستهٔ دوم)
- **#1 دراورِ پشتیبانی در داشبورد:** `SupportCenter` حالتِ `variant="drawer"` گرفت (فقط دراور، بدون کارت) + شنونده‌ی رویدادِ سراسری `open-support-drawer`. لینکِ «پشتیبانی» در PlanTileِ داشبورد دیگر به پروفایل نمی‌رود؛ با `OpenSupportDrawerLink` دراور را در همان صفحه باز می‌کند.
- **#2 خطای ثبتِ استوری:** با ریپروِ زنده تأیید شد که جریانِ استوری سالم است (POST 200 / تکراری 409). خطای اسکرین‌شات از goalِ حذف‌شدهٔ تبِ کهنه بود (404). به‌عنوانِ self-heal: روی 404، `router.refresh()` صفحه را همگام می‌کند.
- **#3 ترازِ لینکِ کارتِ گزارش:** بدنهٔ ReportTile در `flex-1 min-h-0 overflow-hidden` پیچیده شد تا داخلِ قابِ ثابتِ ۲۰۶px کلیپ شود و فوتِ لینک دقیقاً روی لبهٔ پایین (با paddingِ تایل) بنشیند — هم‌تراز با PlanTile (اندازه‌گیری: هر دو ۱۲۶۳px).
- **#5 یکدست‌سازیِ آیکون‌ها:** باقی‌مانده — دامنه‌اش گسترده و سلیقه‌ای است؛ با مالک به‌صورت ویژوال scope می‌شود.

---

## DECISION-103 — سیستمِ یکدستِ آیکونِ هدرِ کارت‌ها (#5)

**تاریخ:** ۲۰۲۶-۰۶-۱۶ — **وضعیت:** اعمال‌شده (mockup-first؛ `mockups/icon-system.html` تأییدشده)

ملاک = آیکونِ کارتِ «هویت و ورود» در پروفایل: قابِ گردگوشهٔ تینت‌دار + آیکونِ خطی. توکنِ مشترکِ
`.tile-ic` + ۶ تینتِ دسته‌ای (sage=رشد/مسیر، ember=نبض، gold=گزارش/زمان، deep-sage=مالی/پلن،
mist=پشتیبانی/حساب، mist2=یادآوری). navbar و آیکون‌های درون‌خطی (شِورون/عقربه) و آواتارِ پرسوناها مستثنا.

**اعمال‌شده:**
- **داشبورد:** هدرِ آیکون‌دار روی هر ۵ تایل (آیکونِ جمع‌ترِ ۳۰px برای تراکم)؛ بدنه‌ها در `flex-1 min-h-0 overflow-hidden` تا قابِ ثابت نشکند (۰ سرریز، تأییدِ زنده).
- **صفحهٔ هدف:** هیرو (tile-ic نوع‌محور)، مسیر (ic-mist)، CompanionMark از دایره به مربع.
- **گزارش هفتگی و پلن‌ها:** سرتیترِ صفحه با tile-ic.

**بلاگ/صفحاتِ استاتیک (CMS):** بررسی شد — این صفحات از قبل آیکون‌های تینت‌دارِ گردگوشه دارند
(مثلِ بخشِ تماس: `w-10 h-10 rounded-xl` با تینتِ sage/ember/gold) و منسجم‌اند؛ تحمیلِ توکنِ دقیقِ
۳۴px به چیدمانِ سفارشیِ سایتِ عمومی churnِ کم‌ارزش است و طبق mockup-first بهتر است جدا با مالک
look-check شود. منتظرِ نظرِ مالک.

---

## DECISION-105 — بازطراحی ساید بار پنل ادمین: گروه‌بندی + مخفی‌کردن اسکرول‌بار

**تاریخ:** ۲۰۲۶-۰۶-۱۷ — **وضعیت:** اعمال‌شده

**مسئله:** آیتم‌های ناوبری ساید بار تخت و شلوغ بودند؛ اسکرول‌بار نیز در صفحه‌های کوچک نمایان بود.

**تصمیم:**
- ساختار `NAV_ITEMS[]` → `NAV_GROUPS[]` (گروه‌ها با عنوانِ ثابت، بدون قابلیتِ جمع/باز)
- ۶ گروه: داشبورد (بدون عنوان) / کاربران / پشتیبانی / مدیریت محتوا / زیر ساخت ها / تنظیمات سایت
- ساید بار از `w-60` به `w-64` (256px) برای جا دادنِ برچسب‌های بلندتر
- کلاسِ `.scrollbar-none` (scrollbar-width:none + webkit) در globals.css برای مخفی‌کردنِ اسکرول‌بار در هر مرورگر
- هر گروه فقط اگر حداقل یک آیتم با permissionِ ادمین داشته باشد نمایش داده می‌شود

---

## DECISION-104 — رفعِ کوپلینگِ ارتفاع (parent مشترک) — داشبورد + هدف + پروفایل

**تاریخ:** ۲۰۲۶-۰۶-۱۷ — **وضعیت:** اعمال‌شده

- **#1 ساعتِ کادر سبز:** زونِ ساعت از `flex:1` به ارتفاعِ طبیعی (۱۱۲px) تغییر کرد و گروهِ [ساعت+خط+هفته] با `margin:auto` وسط‌چین شد. حالا کشیده‌شدنِ پنل (به‌خاطرِ شکل‌های مختلفِ کارتِ تعهد: خالی/ثبت‌شده/فریز/بازتاب) فقط حاشیهٔ بالا/پایین را تغییر می‌دهد، نه فاصلهٔ دورِ ساعت. سایزِ ساعت ۸۴px همیشه ثابت بود.
- **#2 کارتِ تاریخچه:** بازگشت به **۳** تعهدِ اخیر + برشِ متن به **۵۰ کاراکتر + …** (trunc)، و فشرده‌سازیِ ردیف‌ها تا در قابِ ثابت ۰ سرریز بدهد.
- **#3 باکسِ «همراه» (هدف):** `align-self:stretch` تا هم‌ارتفاعِ هیروِ هدف بماند (سایزِ پایدار)، و در bare بدنهٔ بینش `flex-1 min-h-0 overflow-y-auto` با نوارِ مخفی → متنِ بلند داخلِ خودش اسکرول می‌شود (سبکِ همدم) و دیگر هیرو را هل نمی‌دهد.
- **#4 دربارهٔ منِ پروفایل:** دکمهٔ «ویرایش» با `justify-content:space-between` به سمتِ چپ رفت، هم‌تراز با ردیف‌های نام نمایشی و تاریخ تولد.
- **#5 پشتیبانیِ پروفایل:** `.pf-sup-body{align-items:flex-start}` تا کادرِ «نیاز به کمک داری؟» از بالا تراز شود (بدونِ تغییرِ سایز).

### DECISION-104 (اصلاحِ دور دوم — طبق بازخوردِ بصریِ مالک)
- **#1 ساعت (بازنگری):** راهِ درست = `.dsh-top{align-items:start}` تا کادر سبز به ارتفاعِ کارتِ تعهد کشیده **نشود** و همیشه ارتفاعِ طبیعیِ خودش (~۲۴۳px) را داشته باشد → شکل/سایزِ ساعت در هر حالتِ کارت دقیقاً ثابت. (راهِ قبلیِ margin:auto کنار گذاشته شد.)
- **#3 همراه (بازنگری):** اسکرولِ واقعی با الگوی absolute-fill: `.jp-comp{position:relative;padding:0}` + `.jp-comp-fill{position:absolute;inset:0;overflow:hidden}`؛ محتوای پنل دیگر ارتفاعِ ردیف را بزرگ نمی‌کند، پس ردیف = هیرو و بدنه در `flex-1 min-h-0 overflow-y-auto` (نوار مخفی) اسکرول می‌شود. تأییدِ زنده با بینشِ بلند: comp=hero=۴۴۴، اسکرولر ۸۴۶px در ۳۶۴px، سرریزِ باکس ۰. موبایل: fallback به جریانِ طبیعی.
- **#5 پشتیبانی (بازنگری):** بازچینش به دو ستون: `.pf-sup-grid` (راست: سرستون+متن+تیکت‌ها · چپ: کادرِ کمک) با `align-items:flex-start` → کادرِ «نیاز به کمک داری؟» از بالا هم‌ترازِ سرستونِ «ارتباط با پشتیبانی».
- **ارورها:** `/goal 500` و `/api/support/chat/*` 404 از **سرورِ dev کهنه/خراب** (PID یتیم + کشِ Turbopack) بودند، نه باگِ کد؛ روی سرورِ تازه هر دو 200. رفع = ری‌استارتِ تمیزِ dev.

---

### DECISION-105 | بازطراحیِ JourneyRail — فاصله‌گذاریِ پیکان‌ها + رنگِ گرم‌تر
- **تاریخ:** ۱۴۰۵-۰۳-۲۸
- **وضعیت:** ✅ تأیید شده
- **زمینه:** پیکان‌های زنجیره‌ای (RTL) در تایم‌لاینِ هدف روی هم می‌افتادند (CHEV_STEP=74 باعث تداخل ۱۴px می‌شد). همچنین رنگِ قرمز‌ـ‌نارنجی (ember) برای «امروز» حسِ اضطراری/هیستریک داشت.
- **تصمیم:**
  - `CHEV_STEP = CHEV_W` (88 به جای 74) — نوکِ هر پیکان درست به مرزِ شروعِ بعدی می‌رسد، بدونِ تداخل
  - رنگِ «امروز» از `ember` به `gold` تغییر یافت — گرم، مثبت، بدون هیستری
  - `jp-chev-empty` و `jp-leg-chev.future` کمی تیره‌تر شدند (0.16/0.12 به جای 0.2/0.2) — تمایز واضح‌تر
  - drop-shadow روی `.jp-cv-today` به رنگِ gold تبدیل شد

---

### DECISION-106 | جابه‌جاییِ ساعت از dsh-top به بِنتو + تنظیمِ روزهای پلن در ادمین
- **تاریخ:** ۱۴۰۵-۰۳-۲۸
- **وضعیت:** ✅ تأیید شده
- **زمینه:**
  - ساعت/تاریخ کنارِ کارتِ تعهد (dsh-top) چیدمانِ بالای داشبورد را به هم می‌ریخت — کارتِ تعهد باید کانونِ اصلی باشد.
  - ادمین نمی‌توانست روزهای باقیمانده از انقضای پلن کاربر را تنظیم کند.
- **تصمیم (داشبورد):**
  - `dsh-top` به flex تک‌ستونه تبدیل شد؛ `dsh-green-col` مخفی. کارتِ تعهد وسط‌چین.
  - `TodayPanel` به عنوانِ تایلِ `t-week` (span 5×1) در بِنتو جای گرفت، پسِ PulseTile.
  - `.t-goal` از `span 2` به `span 3` ردیف رفت تا جای t-week را در ستونِ چپ باز کند.
  - ترتیبِ جدیدِ بِنتو: `goal (7×3) | pulse (5×1) / week (5×1) / hist (5×1)` + `report (7×1) | plan (5×1)`.
- **تصمیم (ادمین):**
  - بخشِ «تغییر پلن» (UserActions) به ستونِ عریض‌تر (چپ، col-span-7) زیرِ چتِ آنلاین منتقل شد.
  - بخشِ «وضعیت دسترسی» (ban) در ستونِ باریک‌تر (راست، col-span-5) باقی ماند.
  - کنترلِ [−][N روز][+] در UserActions: PATCH `/api/admin/users/[id]/plan-adjust-days`؛ پلنِ دائمی (null) + days>0 → now+days؛ پلنِ دائمی + days<0 → رد؛ انقضاء نمی‌تواند به گذشته برگردد.
- **فایل‌های تغییریافته:**
  - `src/app/globals.css` — DECISION-099/104/105/106 blocks
  - `src/app/dashboard/page.tsx` — حذفِ dsh-green-col، افزودنِ t-week tile
  - `src/components/features/goal/JourneyRail.tsx` — CHEV_STEP fix
  - `src/components/admin/users/UserActions.tsx` — day-adjust UI + planExpiresAt prop
  - `src/app/api/admin/users/[id]/plan-adjust-days/route.ts` — NEW endpoint
  - `src/app/admin/(panel)/users/[id]/page.tsx` — تفکیکِ UserActions به دو نمونه

---

### DECISION-130 | بازپایه‌گذاریِ کاملِ سیستمِ طراحی روی همیار — منبعِ توکنِ واحد
- **تاریخ:** ۱۴۰۵-۰۴-۰۴
- **وضعیت:** 🔄 در جریان — **فاز ۱ (لایت) پایه پیاده شد**؛ فاز ۲ دارک، فاز ۳ ایندیگو ادامه دارد.
- **قاعدهٔ مالک:** ملاکِ مطلق و نهاییِ استایلِ همسو = **همیار**. همسو و همیار در نهایت ادغام می‌شوند و باید از **یک منبع و یک قاعدهٔ کلیِ استایل** بخوانند — هیچ چیزِ استایلی از دو جا خوانده نمی‌شود. پالت = **چندرنگیِ هماهنگ per-theme** (نه مونوکروم، نه حذفِ رنگ): هر رنگ باید با تمِ خودش بنشیند (قرمز در بنفش ناهماهنگ است؛ اما لایت/نبضِ هفته چندرنگیِ کارکردی دارد).
- **استخراج:** کلِ `Hm/hamyar.html` خط‌به‌خط استخراج شد → `docs/hamyar-design-system.md` (مقادیرِ دقیقِ توکن‌ها + CSS هر کامپوننت + انیمیشن‌ها). این فایل منبعِ مرجعِ بازنویسی است.
- **معماریِ منبعِ واحد:** لایهٔ توکنِ معناییِ همیار (`--surface-*`/`--accent-*`/`--accent-gradient`/`--feedback-success/warning/error/info`/`--border-*`/`--shadow-soft/low/mid/high/glow/glow-focus`/`--ambient-*`) در `globals.css` تعریف شد، per `[data-theme]` با مقادیرِ **دقیقِ همیار** (warm-light / warm-dark / indigo-dark). توکن‌های لگاسیِ همسو (`--color-*`, `--rgb-*`, `--shadow-paper*`) به همان نگاشته/الیاس شدند تا کلِ پروژه (اپ + پابلیک + ادمین) بدونِ شکست به همین یک منبع resolve شود.
- **فاز ۱ — لایت (انجام‌شده):**
  - `@theme`: مقادیرِ Tailwind به warm-light همیار (paper #F2EEE4، charcoal #3E3B36، mist-deep #6E8FA5، …) + ایزینگ‌های quiet/spring.
  - پرایمیتیوهای مشترک **token-driven** شدند (خودکار per-theme، منبعِ واحد): `.glass`/`.glass-strong` = `var(--surface-glass)` + `blur+saturate` + `var(--shadow-*)`؛ `.app-blob-*` = `var(--ambient-*)`؛ `.btn-primary` = `var(--accent-gradient)` + `var(--shadow-glow)`.
  - override های اضافیِ per-theme برای این پرایمیتیوها **حذف** شدند (DECISION-128/129) چون توکن خودش فلیپ می‌کند → منبعِ واقعاً واحد.
  - `themeColor` متادیتا → `#F2EEE4`.
- **تستِ بصریِ واقعی (unblocked):** اپِ احرازهویت‌شده با dev server + Playwright (Chrome) و کاربرِ Pro `09120000000` رندر و اسکرین‌شات می‌شود (`mockups/_shots/app/`). دیگر فقط به ماکاپ متکی نیستیم.
- **فاز۲/۳ — حذفِ گرم (انجام‌شدهٔ داشبورد+هدف):** با دیدنِ شاتِ واقعی، اکسنت‌های گرمِ ممنوع (نقطهٔ امروزِ ember، حلقهٔ نبضِ نارنجی، آواتارِ همراهِ gold) در `dsh-/jp-` با override گروهیِ `[data-theme=dark],[data-theme=indigo]` به سرد تبدیل شدند (today→accent-bright، همراه→accent، tint→accent-muted). لایت دست‌نخورده (گرمِ هماهنگ).
- **باقی‌ماندهٔ فازها:** بقیهٔ صفحات (پروفایل/گزارش/تنظیمات/کیف‌پول)، مهاجرتِ کاملِ بقیهٔ هگزهای `dsh-/jp-/pf-` به توکن + افزودنِ افکت‌های زندهٔ همیار (تایم‌لاینِ گرادیان + هاله‌ی pulse، اوراهای متحرک، گلس‌مورفیسمِ هیروها). هر صفحه/کامپوننت اسکرین‌شات + تست.
- **هم‌ترازی/ایمنی:** تغییرات از طریقِ توکن‌ها سراسری‌اند اما مقادیرِ لایت ≈ Warm Paper قبلی (تغییرِ جزئی)؛ ادمین چون از همان `--color-*` می‌خواند، با لایت/دارکِ همیار هماهنگ می‌شود بدونِ شکست. braces ۷۹۲/۷۹۲ · `tsc` exit 0.
- **فایل‌ها:** `src/app/globals.css` (لایهٔ توکن + پرایمیتیوها) · `src/app/layout.tsx` (themeColor) · `docs/hamyar-design-system.md` (NEW، استخراج) · ماکاپِ `mockups/hamyar-dna-dashboard.html` (۳ تم).

---

### DECISION-129 | اعمالِ عمیقِ تمِ ایندیگو — رفعِ «شیشهٔ سفید روی زمینهٔ تیره»
- **تاریخ:** ۱۴۰۵-۰۴-۰۳
- **وضعیت:** ✅ اعمال‌شده (ماکاپِ تأیید: `mockups/indigo-dashboard.html` + شاتِ `_shots/indigo-dashboard.png`)
- **زمینه:** مالک با مقایسهٔ داشبوردِ ایندیگوی همسو با همیار نشان داد ایندیگو «کم‌رنگ/تخت» است — انگار فقط چند توکن منتقل شده. **ریشهٔ فنی:** بلوکِ override کامپوننت‌ها فقط با سلکتورِ `[data-theme="dark"]` نوشته شده بود؛ پس در ایندیگو همهٔ کلاس‌های literal به مقدارِ پیش‌فرضِ روشن برمی‌گشتند — مهم‌تر از همه `.glass` به `rgba(255,255,255,0.55)`. همهٔ تایل‌های داشبورد/پنل (`.glass`) + Today/Pulse/Plan/Report و نوار → کارتِ شیشه‌ایِ **سفید** روی زمینهٔ تیره = ظاهرِ شسته. blobهای محیطی هم sage/gold می‌ماندند (بدون هالهٔ بنفش)، دکمهٔ خانهٔ نوار با گرادیانِ بنفش اما **هالهٔ سبز**، و هگزهای هاردکدِ sage (`#7a8471…`) در تایم‌لاین/ریتم/بَجِ پلن سبز می‌ماندند.
- **تصمیم:** یک بلوکِ override **اسکوپ‌شده به `[data-theme="indigo"]`** در `globals.css` (پس از بلوکِ دارک) که همان کلاس‌ها را با هویتِ بنفش بازتعریف می‌کند:
  - **شیشه‌ها:** `.glass`/`.glass-strong`/`.glass-tinted`/`.glass-nav` → سطحِ `rgba(23,21,51,…)` + لبهٔ لَوَندری + هالهٔ بنفشِ کم.
  - **blobهای محیطی:** `.app-blob-1/2/3` → radial-gradientِ بنفش (`rgba(107,93,211,…)`) — کلیدِ «حسِ ایندیگو».
  - **دکمه‌ها:** `.btn-primary` گرادیانِ بنفشِ همیار (`linear-gradient(135deg,#8E83D9,#6B5DD3)`) + هالهٔ بنفش؛ ghost/glass/pill/step-num/float-card هم بنفش.
  - **داشبورد:** هالهٔ دکمهٔ خانهٔ نوار، `.dsh-tl-link.done`، swatchهای wrote، `.dsh-plan-badge.plus`، `.jp-line-done`، سایهٔ پاپ‌اوورها → بنفش/خنثی. `.fab-glass` هاله‌ی بنفش.
  - **Share modal** کاملِ بنفش.
- **حفظِ عمدی:** اکسنتِ **ember** (امروز/اکشن) و **gold** (همراه) در ایندیگو هم گرم می‌مانند — کنتراستِ هدفمند، نه باگ.
- **هم‌ترازی/ایمنی:** کلِ بلوک با سلکتورِ `[data-theme="indigo"]` → روی **لایت/دارک/ادمین** صفرِ اثر (تضمینِ سلکتور). ایندیگو هم فقط در اپ فعال است (DECISION-128). braces متعادل (۸۰۳/۸۰۳).
- **فایل‌های تغییریافته:** `src/app/globals.css` (بلوکِ ایندیگو + `.fab-glass` ایندیگو) · `mockups/indigo-dashboard.html` (NEW، تأیید بصری).

---

### DECISION-128 | سیستمِ سه‌تم (لایت/دارک/ایندیگو) از پالتِ همیار — حذفِ «سیستم»
- **تاریخ:** ۱۴۰۵-۰۴-۰۳
- **وضعیت:** ✅ تأیید شده (مالک، با ماکاپِ `mockups/themes-three.html`)
- **زمینه:** مالک خواست تمِ همسو با همیار یکپارچه شود + تمِ سومِ **ایندیگو (بنفش)** اضافه شود. جایگزینِ DECISION-067 (که system/light/dark بود).
- **تصمیم:**
  - **سه تم:** لایت (پیش‌فرض) / دارک / ایندیگو. **«system» حذف شد** (پیش‌فرضِ نبودِ مقدار = light).
  - **پالت‌ها از همیار:** دارک ← `warm-dark` (bg #100F0B، sage #A4AD9C، …)؛ ایندیگو ← `indigo-dark` (bg #0B0A18، accent بنفش #8E83D9، …). لایت ≈ همان warm-light (بی‌تغییر چون از قبل هم‌سو بود). در `globals.css`: `[data-theme="indigo"]` افزوده؛ `[data-theme="dark"]` به‌روز شد.
  - **اسکوپِ ایندیگو (فقط اپ):** در `lib/theme` تابعِ `resolveTheme(pref, pathname)` روی مسیرهای پابلیک/ادمین (`isNoIndigoPath`) ایندیگو را به **دارک** تنزل می‌دهد → `data-theme` آن‌جا هرگز "indigo" نمی‌شود. پس بدونِ نیاز به اسکوپِ CSS، پورتال‌ها هم درست‌اند. `ThemeScript` (inline، بدون فلش) و `ThemeRouteSync` (همگام با ناوبریِ کلاینت) همین منطق را دارند.
  - **تاگل:** `ThemeToggle` پراپِ `allowIndigo` گرفت. AppNav → `allowIndigo` (۳ گزینه). LandingNav (پابلیک) و AdminShell (ادمین) → ۲ گزینه (لایت/دارک). آیکونِ ایندیگو افزوده شد.
  - **ایندیگو روی dark: تیلویند:** `@custom-variant dark` گسترش یافت تا `[data-theme="indigo"]` هم واریانتِ `dark:` را فعال کند (چون تمِ تیره است).
- **پیامدها:**
  - پابلیک: لایت/دارک (بدون ایندیگو). ادمین: لایت/دارک، هرگز ایندیگو.
  - **یادداشتِ شفاف:** مقادیرِ دارک حالا سراسری از پالتِ warm-dark همیار می‌آیند → دارک‌مودِ ادمین هم همان تونِ گرم را می‌گیرد (تغییرِ جزئی، نه مخرب). اگر مالک تونِ دارکِ قبلیِ ادمین را بخواهد، با مارکرِ `data-admin` قابلِ اسکوپ است.
  - تأیید: braces ۷۶۰/۷۶۰ + `tsc` سبز.

---

### DECISION-127 | ⚖️ قاعدهٔ حاکمیتی — نظرِ مالک مطلق، مانیفست/برندبوک دیگر مرجعِ الزام‌آور نیست
- **تاریخ:** ۱۴۰۵-۰۴-۰۳
- **وضعیت:** ✅ قطعی (غیرقابلِ بحثِ مجدد)
- **تصمیمِ صریحِ مالک:** از این پس هرچه مالک بگوید مرجعِ مطلق است؛ دیگر به مانیفست/برندبوک استناد نمی‌شود و هیچ تعارضی با آن مطرح نمی‌گردد. برندبوک صرفاً منبعِ الهام است.
- **پیامدها:** جایگزینِ قاعدهٔ قبلیِ CLAUDE.md §۱. ثبت در CLAUDE.md + حافظهٔ بلندمدت (`owner_overrides_manifest`). هرگز دیگر «تعارض با مانیفست» مطرح نمی‌شود.

---

### DECISION-126 | رفعِ کلیپِ حبابِ تایم‌لاین (ریشهٔ واقعی) + شیشه‌ایِ واقعی FAB + رنگِ بژِ پشتیبانی
- **تاریخ:** ۱۴۰۵-۰۴-۰۳
- **وضعیت:** ✅ تأیید شده (اثباتِ بصری: `mockups/_shots/timeline-proof.png`)
- **زمینه:**
  - مالک چندین‌بار تذکر داد که **حبابِ استوریِ تایم‌لاینِ هدف روی موبایل دیده نمی‌شود**. اصلاحاتِ قبلی (hover→tap→press-hold) رفتارِ تریگر را درست کرد ولی حباب همچنان دیده نمی‌شد.
  - **ریشهٔ واقعی (یافته شد):** در DECISION-116 روی `.dsh-tl` موبایل `overflow-x:auto` گذاشته بودم. طبق اسپکِ CSS، وقتی یک محورِ overflow غیر از visible شود، محورِ دیگر هم به `auto` **القا می‌شود** → `.dsh-tl-pop` که در `bottom:170%` (بالای نقطه) می‌نشیند، عمودی **کلیپ** می‌شد. این علتِ نهاییِ «دیده‌نشدن» بود (مستقل از تریگر).
  - دو نکتهٔ دیگر: `.fab-glass` با کدورتِ ۰.۵۵ زیرش پیدا نبود (مالک: «شیشه‌ای یعنی زیرش پیدا باشد»)؛ دکمهٔ پشتیبانی در ری‌فکتور سفید مانده بود (باید بژ/طلایی باشد).
- **تصمیم:**
  - **حذفِ `overflow-x:auto` از `.dsh-tl` موبایل** (نقاط windowed‌اند ≤۸ و جا می‌شوند) → پاپ‌اوور دیگر کلیپ نمی‌شود و با press-hold ظاهر می‌شود. اثباتِ بصری ضمیمه.
  - **`.fab-glass`:** کدورتِ پس‌زمینه ۰.۵۵→۰.۲۲ + بلورِ قوی‌تر (۲۲px/۱۸۰٪) → شیشهٔ واقعی که محتوای زیر از آن پیداست.
  - **دکمهٔ پشتیبانی:** تینتِ بژ/طلایی `rgba(193,154,74,0.22)` (نه سفید) — هم‌رنگِ قبل.
- **پیامدها:** تأیید: braces ۷۵۹/۷۵۹ + `tsc` سبز. اثباتِ بصری در ریپرو. (نکتهٔ جزئیِ باقی: پاپ‌اوورِ نقطهٔ لبه‌ای ممکن است کمی از لبهٔ صفحه بیرون بزند — در صورتِ نیاز position لبه‌آگاه می‌شود.)

---

### DECISION-125 | FABِ چت — یک پلاسِ شیشه‌ایِ گسترش‌یابنده به‌جای دو FAB (سبکِ iOS Liquid Glass)
- **تاریخ:** ۱۴۰۵-۰۴-۰۳
- **وضعیت:** ✅ تأیید شده (مالک، با ماکاپِ `mockups/fab-speeddial.html`)
- **زمینه:**
  - ایدهٔ مالک: به‌جای دو FABِ همیشه‌نمایان، یک **پلاسِ شیشه‌ای** که با لمس به چت‌ها گسترش یابد.
  - بنچ‌مارک تأییدش کرد: راهنمای اپل «یک صفحه تقریباً هرگز نباید بیش از یک FAB داشته باشد»؛ iOS 26 «Liquid Glass» دقیقاً برای کنترل‌های شناورِ شیشه‌ای. منابع: Apple HIG، uxplanet «iOS 26 Beyond Liquid Glass».
- **تصمیم (`FloatingActions` + globals `.fab-glass`/`.fab-pill`):**
  - **PRO (۲ چت):** دکمهٔ اصلی = پلاسِ شیشه‌ای؛ لمس → `+` با spring به `×` می‌چرخد و دو اکشنِ شیشه‌ای (همدم + پشتیبانی، با برچسبِ pill و springِ staggered) بیرون می‌جهند + پس‌زمینهٔ محو. لمسِ هرکدام → چتِ حبابی (DECISION-124). بستن = جمع‌شدنِ نرم به `+`. badgeِ خوانده‌نشده روی پلاس وقتی جمع است.
  - **رایگان (۱ چت):** دکمهٔ اصلی = حبابِ همدم؛ لمس → **مستقیم** همدم باز می‌شود (بدونِ گسترش — speed-dial برای یک اکشن ضدالگوست).
  - شیشه‌ایِ دارک‌اگاه (`var(--rgb-card/line)`)؛ پلاس کم‌رنگِ ۶۰٪ تا لمس روی دستگاهِ لمسی (`.chat-fab`).
- **پیامدها:** جایگزینِ دو FABِ DECISION-118/124 (همان منطقِ unread/online/eventِ open-support-chat حفظ شد). دسکتاپ و پنل ادمین بی‌اثر. تأیید: braces ۷۶۱/۷۶۱ + `tsc` سبز.

---

### DECISION-124 | چتِ موبایل — حبابِ کارتیِ سبکِ iOS + FABهای کم‌رنگ تا لمس
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده (مالک، با ماکاپِ `mockups/chat-mobile.html`)
- **زمینه:**
  - پنجره‌های چت در موبایل تمام‌عرض و تمام‌قد (`bottom-0 inset-x-0 w-full h-[85dvh]`) باز می‌شدند — مالک آن را زیبا نمی‌دانست. بنچ‌مارک (iOS/کلاس‌جهانی): الگوی مدرن = bottom sheet با spring + ورودیِ چسبیده، نه takeoverِ تمام‌صفحه. منابع: Apple UISheetPresentationController، UXPlanet/bricxlabs chat patterns.
  - FABها همیشه پررنگ بودند؛ مالک خواستِ «کم‌رنگ تا لمس» (هم‌سو با «سکوتِ بصری»ِ مانیفست).
- **تصمیم:**
  - **ChatWindow + SupportChatWindow:** موبایل (<md) → **حبابِ کارتی**: `inset-x-2.5 bottom-[74px+safe] h-[78dvh] rounded-3xl`، با `transform-origin:bottom right` و انیمیشنِ **spring** (`cubic-bezier(.34,1.4,.5,1)`، `scale(.4)→scale(1)`) → حسِ «باز شدن از FAB». دسکتاپ (md+) بدونِ تغییر (پنلِ گوشه). نقطه‌شکست از `sm`→`md` تا با حضورِ نوارِ تب هماهنگ شود.
  - **FABها (`FloatingActions`):** کلاسِ `chat-fab`؛ روی دستگاهِ لمسی `@media(hover:none){opacity:.6; :active→1}`؛ opacity در حالتِ نمایان از CSS (inline = `undefined`). موقعیت کمی بالاتر (همدم ۵.۲۵rem، پشتیبانی ۹rem).
- **پیامدها:** هر دو پنجرهٔ چت یکسان. دسکتاپ و پنل ادمین بی‌اثر. تأیید: braces ۷۵۸/۷۵۸ + `tsc` سبز. تستِ بصریِ نهاییِ مالک.

---

### DECISION-123 | پاپ‌اوورِ استوریِ داشبورد لمس‌محور (رفعِ «استوری‌ها روی موبایل دیده نمی‌شوند»)
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده
- **زمینه:** استوری‌ها روی **تایم‌لاینِ هدف** (داشبورد) و **حباب‌های تقویمِ هفته** (کارتِ ساعت) فقط با `hover`/`mouseEnter` ظاهر می‌شدند — که روی موبایل **اصلاً اجرا نمی‌شود** (کاربر لمس می‌کند نه hover). علتِ «استوری‌ها روی تایم‌لاین دیده نمی‌شوند».
- **تصمیم (اصلاح‌شده — رفتارِ press-hold، نه tap-toggle):**
  - بازخوردِ مالک: tap-toggle حباب را می‌چسباند و با اسکرول جابه‌جا می‌کرد. رفتارِ درست = **نگه‌داشتنِ انگشت** نمایش می‌دهد، **رهاکردن** فوراً پنهان می‌کند (مثلِ peekِ موبایل).
  - `GoalTile` (کلاینت): `onPointerDown` (لمسی) → `setOpenIdx(i)`؛ `onPointerUp/Leave/Cancel` → پنهان. `pointerType` تفکیک: دسکتاپ از `:hover` CSS (دست‌نخورده)، موبایل از نگه‌داشتن. `touchAction:pan-y` تا اسکرول حفظ شود؛ شروعِ اسکرول → `pointercancel` → پنهان (رفعِ باگِ چسبیدن).
  - `TodayPanel`: `onPointerEnter` (ماوس=hover) + `onPointerDown` (لمس=نگه‌داشتن) → نمایش؛ `onPointerUp/Leave/Cancel` → پنهان.
- **پیامدها:** حباب فقط حین نگه‌داشتن دیده می‌شود و با رها/اسکرول می‌رود؛ دیگر نمی‌چسبد. تأیید: `tsc` سبز.

---

### DECISION-122 | فاز A1 — پاسِ ریسپانسیوِ باقیِ صفحاتِ موبایل + امن‌سازیِ ارتفاعِ مودال‌ها
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده (مالک تستِ نهایی می‌گیرد)
- **زمینه:**
  - پویشِ سیستماتیکِ ریسکِ موبایل روی صفحاتِ باقی‌مانده (settings · reports · support · story · onboarding + مودال‌ها). یافته:
    - صفحاتِ احرازهویت‌شدهٔ باقی‌مانده **Tailwind‌اند** (grid-colsشان `minmax(0,1fr)` است → بدونِ grid blowout)، **بدونِ جدول و بدونِ عرضِ ثابتِ خطرناک** → ریسپانسیو. (جدول‌ها فقط در پنل ادمینِ دسکتاپ و /plansِ عمومی‌اند.)
    - پنجره‌های چت (`ChatWindow`/`SupportChatWindow`) از قبل bottom-sheetِ تمام‌عرض در موبایل‌اند (`w-full sm:w-95`).
    - `/notifications` صفحه‌ای ندارد (از `NotificationBell` می‌آید). `story`/`onboarding` عمداً بدونِ AppShell/نوارِ تب‌اند (جریانِ تمرکزی).
    - **تنها ریسکِ واقعی:** مودال‌های `w-full max-w-md/sm` با `overflow-hidden` و **بدونِ `max-height`** → روی صفحهٔ کوتاه، محتوای بلند بریده می‌شد (نه اسکرول).
- **تصمیم:**
  - **امن‌سازیِ ۶ مودال** (DayDetail · GoalEdit · GoalDelete · ReminderSettings · Freeze · RecentHistory): `overflow-hidden` → `max-h-[calc(100dvh-2.5rem)] overflow-y-auto` → محتوای بلند داخلِ خودِ مودال اسکرول می‌شود (هم‌سو با قانونِ بدون‌اسکرول‌بارِ globals).
  - بقیهٔ صفحات **عمداً دست‌نخورده** ماندند (از قبل ریسپانسیو؛ طبق هشدارِ مالک «بخشِ سالم را نشکن»).
- **پیامدها:** A1 کامل. تأیید: `tsc` سبز. AvatarCrop/SetPassword/Share عمداً مستثنا (ساختارِ ارتفاع‌حساس/گاردِ جدا دارند). تستِ بصریِ نهایی با مالک. → بعدی: epicِ **همیار** (با سندِ معماری اول).

---

### DECISION-121 | نصب‌پذیریِ PWA — manifest + آیکون‌ها + متادیتای iOS + Service Worker
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده (مالک: «اول PWA را کامل کن»)
- **زمینه:**
  - بررسی نشان داد اپ **هیچ زیرساختِ PWA نداشت** (نه manifest، نه service worker، نه آیکونِ PWA، نه next-pwa) → «نصب روی گوشی ممکن نیست» (TASK-5). صرفاً ریسپانسیو بود، نه نصب‌پذیر.
- **تصمیم (فاز A2 از ترکِ PWA):**
  - **آیکون‌ها:** از `public/logo.png` (wordmark ۵۶۱×۳۶۵) با `sharp` روی پس‌زمینهٔ Warm Paper ساخته شد: `icon-192/512` + `*-maskable` (safe-zone بیشتر) + `apple-touch-icon-180`. **آیکونِ اولیه** — قابلِ تعویض با آیکونِ نمادینِ اختصاصی بعداً.
  - **manifest:** `src/app/manifest.ts` (Next 16 بومی → `/manifest.webmanifest` + لینکِ خودکار). display:standalone، lang:fa، dir:rtl، start_url:/dashboard، theme/background: `#F5F2EB`.
  - **متادیتا (layout):** `appleWebApp` (نصبِ iOS)، `apple` icon، `viewport.themeColor`، `formatDetection.telephone:false`.
  - **Service Worker (`public/sw.js`):** حداقلی و **محافظه‌کار** — ناوبری‌ها **network-first** (هرگز صفحهٔ کهنه)، فقط داراییِ استاتیکِ هش‌دارِ build + فونت/آیکون cache-first، fallbackِ `/offline`.
  - **ثبت‌کننده (`ServiceWorkerRegistrar`):** فقط `IS_PROD_MODE` ثبت می‌کند؛ در dev **unregister** می‌کند تا کشِ Turbopack کهنه نشود.
  - **صفحهٔ `/offline`:** ساده، لحنِ مانیفستی، در `PUBLIC_PATHS`.
  - **proxy:** `/sw.js`،`/manifest.webmanifest`،`/icons/`،`/apple-touch-icon.png` به SKIP_PREFIXES و `/offline` به PUBLIC_PATHS.
- **پیامدها:**
  - SW فقط در production فعال است → dev بدونِ دردسرِ کش. تستِ واقعی: `npm run build && npm start` (با APP_MODE=production) + DevTools → Application/Lighthouse.
  - آیکونِ wordmark اولیه است؛ در صورتِ تمایلِ مالک با آیکونِ نمادین جایگزین می‌شود.
  - تأیید: `tsc` سبز. باقیِ ترکِ PWA = A1 (ریسپانسیوِ صفحاتِ باقی‌مانده + مودال‌ها + QA).

---

### DECISION-120 | رفعِ سرریزِ افقیِ صفحهٔ goal در موبایل + پاک‌سازیِ ابزارهای dev از روی نوارِ تب
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده (بر پایهٔ اسکرین‌شاتِ مالک از goal@430px)
- **زمینه:**
  - مالک با اسکرین‌شات نشان داد در `goal@430px` کارت‌ها (همراه + ریلِ سفر) سرریزِ افقی دارند و کلِ گرید جابه‌جا/بریده است.
  - **ریشه (grid blowout):** `grid-template-columns:1fr` معادلِ `minmax(auto,1fr)` است؛ کفِ `auto` اجازه می‌دهد فرزندِ عریض — `jp-rail-track` با `width:max-content` — ستون را تا عرضِ محتوای ریل منبسط کند → کلِ گرید از viewport پهن‌تر می‌شود (بنچ‌مارک: CSS-Tricks «Preventing a Grid Blowout»).
  - دو نکتهٔ دیگرِ اسکرین: ابزارهای dev (`DevModeBadge` + `DevResetPanel`، هر دو `fixed bottom-3 left-*`) و نشانگرِ dev خودِ Next.js 16.2.6 (لوگوی N، پایین-چپ) روی تبِ پروفایلِ نوارِ پایین می‌افتادند.
- **تصمیم:**
  - **goal overflow (globals.css):** در ≤۷۶۰، `.dsh-bento`/`.jp-bento` → `grid-template-columns:minmax(0,1fr)` + `min-width:0` روی `.jp-tile` و `.jp-rail-outer/.jp-rail-vp` → اسکرولِ داخلیِ ریل فعال می‌شود به‌جای سرریزِ صفحه. همین برای `.pf-bento-cols` در ≤۸۶۰.
  - **نشانگرِ Next (next.config.ts):** `devIndicators: false` (روشِ مستندِ Next 16؛ overlayِ خطاها سالم می‌ماند).
  - **ابزارهای dev:** `DevModeBadge` و `DevResetPanel` با `hidden md:block` فقط روی موبایل مخفی شدند — **روی دسکتاپ حفظ‌اند** (غیرمخرب؛ ابزارهای §۱۳ CLAUDE.md برای توسعه دست‌نخورده می‌مانند). در prod هم اصلاً رندر نمی‌شوند.
- **پیامدها:**
  - اگر مالک مخفی‌سازیِ موبایل کافی نبود و حذفِ کاملِ آن دو ابزار را خواست، با حذفِ دو خط در `layout.tsx` ممکن است (اما توصیه نمی‌شود چون workflowِ dev را می‌برد).
  - تأیید: braces ۷۵۳/۷۵۳ + `tsc` سبز. نیازمندِ اسکرین‌شاتِ مجددِ مالک از goal برای تأییدِ رفعِ سرریز.

---

### DECISION-119 | نوارِ ناوبریِ پایینِ یکپارچهٔ همسو⇄همیار — ۵ تب با دکمهٔ مرکزیِ برجسته
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده (مالک، با ماکاپِ `mockups/nav-unified.html`)
- **زمینه:**
  - نوارِ تبِ موبایلِ قبلی (DECISION-117) ۴‌تبهٔ تخت بود؛ مالک آن را «ابتدایی» دانست و خواستِ سبکِ همیار + کیفیتِ اپ‌های برترِ دنیا را داد.
  - **چشم‌اندازِ ادغام:** همسو (مسیرِ شخصی) و همیار (شبکهٔ اجتماعیِ فاز ۳) هر دو مستقلاً bottom-nav دارند؛ راه‌حل = **یک نوارِ پایینِ مشترک** برای کلِ اکوسیستم.
  - استخراج از کدِ همیار (`Hm/hamyar.html` خطوط ۱۰۴۶–۱۰۹۰): آیتمِ مرکزی یک **دکمهٔ گردِ برجسته (raised)** با گرادیانِ accent + سایهٔ نور + بوردرِ هم‌رنگِ پس‌زمینه است. بنچ‌مارک: Material/UXPlanet — ۳ تا ۵ تب، حالتِ active متمایز، آیکونِ آشنا، FAB مرکزیِ برجسته (منابع در پاسخ).
- **تصمیم (`AppNav.tsx` + globals `.app-botnav/.app-bn*` (DECISION-119)):**
  - ۵ تب، راست→چپ: **مسیر (`/goal`) · گزارش (`/reports/weekly`) · [خانه (`/dashboard`) = مرکزِ برجسته] · همیار(به‌زودی، غیرفعال) · پروفایل (`/settings/profile`)**.
  - دکمهٔ مرکزیِ خانه: دایرهٔ ۵۶px، گرادیانِ `sage→sage-deep`، `margin-top:-22px` (بالاتر از نوار)، بوردرِ ۳px هم‌رنگِ paper، سایهٔ نور. عینِ `bnav-home` همیار اما با پوستِ Warm Paper.
  - تبِ **همیار** فعلاً `disabled` با نشانِ «به‌زودی» (طلایی). **مدلِ ادغام (آینده):** زدنِ همیار → واردِ دنیای همیار؛ آن‌جا منوهای بالاییِ همیار (خانه/چالش‌ها/کلاب‌ها/کتابخانه/آینه) در **بالا** ظاهر می‌شوند و **همین نوارِ پایین** ثابت می‌ماند. (تأییدِ صریحِ مالک.)
  - دارک‌اگاه با `var(--rgb-paper/--rgb-line)`. فقط موبایل (`@media(min-width:768px){display:none}`).
  - AppShell pb به `4rem` رسید (فضای دکمهٔ برجسته که بالای نوار بیرون می‌زند).
- **پیامدها:**
  - جایگزینِ کاملِ نوارِ ۴‌تبهٔ DECISION-117؛ «پلن‌ها» در نوار نیست (از PlanTile/پروفایل در دسترس).
  - تأیید: braces ۷۴۶/۷۴۶ + `tsc` سبز. نیازمندِ تأییدِ بصریِ واقعیِ مالک در موبایل.
  - باقی‌مانده: پیاده‌سازیِ واقعیِ دنیای همیار (فاز ۳) که تبِ همیار را فعال و منوهای بالاییِ آن را اضافه می‌کند.

---

### DECISION-118 | فاز B — ریسپانسیوِ صفحاتِ هدف/پروفایل + clearanceِ نوارِ تب + بالا‌بردنِ FABها
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده (نیازمندِ تأییدِ بصریِ مالک در ۴۳۰px)
- **زمینه:**
  - ادامهٔ ری‌دیزاینِ موبایل (DECISION-116/117) برای صفحاتِ `goal` (jp-) و `profile` (pf-).
  - اسکرین‌شاتِ مرجع نشان داد ماکاپ‌های قدیمیِ `goal-journey.html`/`profile-redesign.html` خودشان در ۴۳۰px سرریزِ افقی دارند (دسکتاپ‌محورند) → مرجعِ تمیزِ موبایل = همیار + `app-mobile-dashboard.html`، نه آن‌ها. صفحاتِ واقعی چون قواعدِ تک‌ستونهٔ ≤۷۶۰ دارند بهترند، اما اندازه‌های px ثابت و گریدِ `minmax(240px,…)` همچنان ریسکِ سرریز داشتند.
  - نوارِ تبِ پایینِ موبایل (fixed) می‌توانست محتوای کفِ صفحات و FABهای چت (`FloatingActions`) را بپوشاند.
- **تصمیم (افزایشی و اسکوپ‌شده؛ دسکتاپ بی‌تغییر):**
  - **globals.css:** سیّال‌سازیِ `padding-inline`/`padding`ِ `jp-wrap`/`pf-wrap`/`jp-tile`/`pf-tile` با clamp (سقف = دسکتاپ)؛ `jp-journeys` → `minmax(min(240px,100%),1fr)` (ضدِسرریزِ کتابخانهٔ مسیرها)؛ روی موبایل کاهشِ `padding-bottom`ِ سه wrap به ۲۰px.
  - **AppShell:** `pb-[calc(3.5rem+env(safe-area-inset-bottom))] md:pb-0` روی کانتینرِ محتوا → clearanceِ یکنواختِ نوارِ تب برای **همهٔ** صفحاتِ احرازهویت‌شده (نه فقط dsh/jp/pf).
  - **FloatingActions:** FABِ همدم و پشتیبانی روی موبایل به بالای نوارِ تب منتقل شدند (`bottom-[calc(...+env(safe-area-inset-bottom))]`)، روی md+ به موقعیتِ اصلی برمی‌گردند.
- **پیامدها:**
  - هر صفحهٔ احرازهویت‌شدهٔ جدید خودکار clearanceِ کفِ تب را از AppShell می‌گیرد.
  - تأیید: braces متوازن (۷۳۰/۷۳۰) + `tsc --noEmit` سبز.
  - باقی‌مانده‌ها: تأییدِ بصریِ واقعیِ مالک در ۴۳۰px (dashboard/goal/profile) + در صورتِ نیاز ساختِ ماکاپِ موبایلِ اختصاصیِ goal/profile + انتقالِ خروج به صفحهٔ پروفایل.

---

### DECISION-117 | ناوبریِ موبایل — جایگزینیِ همبرگر با نوارِ تبِ پایین (مطابقِ ماکاپ/همیار)
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده
- **زمینه:**
  - ماکاپِ تأییدشدهٔ `mockups/app-mobile-dashboard.html` و مرجعِ همیار (`Hm/hamyar.html`) هر دو روی موبایل **نوارِ تبِ پایین** (bottom-tab، شست‌محور) دارند، نه همبرگر. اسکرین‌شات‌های مرجع: `mockups/_shots/hamyar-430.png` و `mockup-430.png`.
  - پیاده‌سازیِ قبلی (فاز A) همبرگرِ `AppNav` را نگه داشته بود → عدمِ تطابق با ماکاپِ مورد‌توافق (بازخوردِ صریحِ مالک: «ماکاپ = قرارداد؛ عیناً همان پیاده شود»).
- **تصمیم (`src/components/layout/AppNav.tsx`):**
  - **موبایل (<md):** همبرگر و منوی کشویی حذف شد؛ `<nav>`ِ دومِ `fixed bottom-0 z-50 glass-nav` با ۴ تب: خانه/چالش/گزارش/پروفایل (مطابقِ ماکاپ). `padding-bottom: env(safe-area-inset-bottom)` برای PWA. نشانِ ember برای تبِ فعال.
  - **نوارِ بالا روی موبایل:** فقط برند + ناقوس + تم؛ به‌علاوهٔ یک آیکونِ «خروج» (چون خروج پیش‌تر فقط داخلِ منوی همبرگری بود و نباید حذف می‌شد — جای‌گذاریِ نهایی‌اش در فاز B قابلِ انتقال به صفحهٔ پروفایل است).
  - **دسکتاپ (md+):** کاملاً دست‌نخورده (ناوبریِ افقیِ مرکز + پروفایل + خروجِ متنی).
- **پیامدها:**
  - تب‌ها از `isActive` مشترک استفاده می‌کنند؛ `/plans` در نوارِ تب نیست ولی از PlanTileِ داشبورد و پروفایل در دسترس است.
  - **برای فاز B:** ① بررسیِ تداخلِ FABِ چت (`FloatingActions`) با نوارِ تب در موبایل · ② اطمینان از clearanceِ کفِ همهٔ صفحاتِ احرازهویت‌شده (نه فقط dsh/jp/pf که pb~۸۰–۹۰ دارند) · ③ انتقالِ خروج به صفحهٔ پروفایل.
  - **نیازمندِ تأییدِ بصریِ مالک** در ۴۳۰px (لاگین‌شده).

---

### DECISION-116 | هاردنینگِ ریسپانسیوِ داشبورد (فاز A از ری‌دیزاینِ موبایل؛ مرجعِ زیبایی = همیار)
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده (فاز A — جراحیِ امن)
- **زمینه:**
  - پس از تبدیلِ اپ به PWA، صفحاتِ احرازهویت‌شده زیرِ ۷۶۸px و به‌ویژه ۴۳۰px (موبایلِ استاندارد) بهم‌ریختگیِ شدید داشتند؛ صفحاتِ عمومی سالم بودند.
  - ماکاپِ تک‌صفحه‌ایِ «همیار» (`Hm/hamyar.html`) در هر سایزی تمیز رندر می‌شد. بررسیِ فنی نشان داد علتِ تفاوت **معماریِ presentation** است نه منطق:
    - **همسو:** بِنتوی دسکتاپ با `grid-auto-rows: 206px` (قابِ ارتفاعِ ثابت) + تنها یک نقطه‌شکستِ ۷۶۰ + اندازه‌های px ثابت → ردیف‌های افقیِ ۷خانه‌ای (نبض/هفته) و تایم‌لاین سرریز می‌کردند و محتوا با line-clamp بریده می‌شد.
    - **همیار:** موبایل‌اول، تایپوگرافیِ سیّال `clamp()`، نردبانِ نقطه‌شکست (۱۲۴۰/۹۸۰/۶۸۰)، `minmax(0,1fr)` + `min-width:0`، ارتفاعِ محتوا‌محور.
  - تأییدِ مالک: «ملاکْ از هر نظر همیار است» — و مودهای `warm-light`/`warm-dark`ِ همیار **عیناً همان Warm Paperِ همسو** هستند (accent=sage، ember، gold، mist)، پس تعارضی با برندبوک نیست. مودِ بنفشِ `indigo-dark` فقط پوستِ تستی بود.
- **تصمیم (فاز A — افزایشی و برگشت‌پذیر):**
  - بلوکِ هاردنینگِ اسکوپ‌شده به `.dsh-*` در انتهای بخشِ داشبوردِ `globals.css`؛ قواعدِ دسکتاپِ موجود دست‌نخورده.
  - **سیّال‌سازی:** `padding-inline` کانتینر و `padding`/`min-width:0`ِ تایل‌ها با `clamp()` که سقفشان = مقدارِ فعلیِ دسکتاپ → دسکتاپ بی‌تغییر.
  - **نردبانِ نقطه‌شکست:** ≤۶۳۹ تک‌ستونه · ۶۴۰–۹۸۰ دو‌ستونهٔ محتوا‌محور (`repeat(2,minmax(0,1fr))` + `grid-auto-rows:auto`) · ≥۹۸۱ بِنتوی ۱۲ستونهٔ فعلی.
  - **ضدِسرریزِ موبایل ≤۷۶۰:** `min-width:0` روی `.dsh-rh`/`.dsh-wcell`، gapِ سیّال، پیل/ساعت/دیجیتالِ سیّال، و `overflow-x:auto`ِ تایم‌لاین (`.dsh-tl`).
- **دلیل:** خاموش‌کردنِ فوریِ «آتشِ موبایل» بدونِ ریسکِ بازنویسی؛ منطقِ سرور/JSX دست نمی‌خورد. مسیرِ بعدی فاز B (نوارِ تبِ پایین + نزدیک‌ترشدن به DNAی همیار) است.
- **پیامدها:**
  - **هم‌ترازی حفظ شد:** تغییرات اسکوپ‌شده به `.dsh-*`اند؛ هیچ قاعدهٔ سراسریِ مشترک با پنل ادمین (ریستِ اسکرول‌بار، `user-select`) لمس نشد.
  - ماکاپِ مرجع: `mockups/app-mobile-dashboard.html` (ثبت در README پوشه).
  - گام‌های بعدی (فاز B): اعمالِ همان الگو روی `goal` (jp-) و `profile` (pf-) + ناوبریِ نوارِ پایینِ موبایل + تأییدِ بصریِ مالک در سایزهای مختلف.
  - **نیازمندِ تأییدِ بصری:** چون رندرِ موبایل روی این سیستم به‌سختی قابلِ‌مشاهده است، مالک باید داشبورد را در ۴۳۰px/تبلت ببیند و بازخورد دهد.

---

### DECISION-115 | صفحاتِ عمومی — منوی پلن‌ها + صفحهٔ قوانین + layout دوگانهٔ /plans
- **تاریخ:** ۱۴۰۵-۰۴-۰۲
- **وضعیت:** ✅ تأیید شده
- **زمینه:** درخواست افزودن «پلن‌ها» به منوی عمومی + صفحهٔ مستقل «قوانین و مقررات» + بازنگری فوتر.
- **تصمیم:**
  - **منوی LandingNav:** «پلن‌ها» بین «تماس با ما» و «بلاگ» اضافه شد (ترتیب RTL: صفحه اصلی · درباره ما · تماس با ما · پلن‌ها · بلاگ).
  - **فوتر LandingFooter:** ستونِ «لینک‌های مفید» بازطراحی شد — «درباره ما» و «تماس با ما» حذف (چون در منو هستند)؛ به‌جای آن «قوانین و مقررات» → `/terms` اضافه شد. ترتیب: داستان همسو · حریم خصوصی · قوانین و مقررات.
  - **صفحهٔ /terms:** صفحهٔ عمومیِ مستقل با CmsPageShell ساخته شد. محتوا از TermsModal استخراج و به TermsContent کامپوننت مشترک تبدیل شد (modal + page هر دو از TermsContent استفاده می‌کنند).
  - **Layout دوگانهٔ /plans (DECISION-040 توسعه‌یافته):** `/plans` حالا conditional است — بدونِ لاگین: CmsPageShell + PricingPublicView (فقط مقایسه، بدون خرید)؛ لاگین‌کرده: AppShell + PlansPricing (مدیریت کامل).
  - **PricingPublicView:** کامپوننتِ جدید با ۳ بخش: کارت‌های قیمت با toggle ماهانه/سالانه (+ نشانِ درصد صرفه‌جویی)، جدولِ مقایسهٔ کامل امکانات ردیف‌محور، سوالاتِ متداول accordion. بنچ‌مارک: ۳ tier (بهترین conversion)، annual toggle (25-35% uptake) — منابع: webstacks.com، eleken.co، lollypop.design (2025).
  - **/terms در PUBLIC_PATHS:** به `src/proxy.ts` اضافه شد.
- **پیامدها:**
  - TermsModal محتوا را به TermsContent delegate می‌کند → یکپارچگی محتوا.
  - `/plans` یک URL برای هر دو حالت → UX استاندارد SaaS (مثل Notion، Linear).
  - جدولِ مقایسه از `COMPARE_ROWS` استاتیک است — اگر امکانِ جدیدی به PLAN_FEATURES اضافه شد، باید `COMPARE_ROWS` در PricingPublicView هم آپدیت شود.
- **فایل‌های تأثیرگرفته:**
  - `src/components/layout/LandingNav.tsx` — NAV_LINKS
  - `src/components/layout/LandingFooter.tsx` — ستونِ لینک‌های مفید
  - `src/app/terms/page.tsx` — NEW
  - `src/components/features/terms/TermsContent.tsx` — NEW
  - `src/components/features/auth/TermsModal.tsx` — delegate به TermsContent
  - `src/app/plans/page.tsx` — layout دوگانه
  - `src/components/features/plans/PricingPublicView.tsx` — NEW
  - `src/proxy.ts` — PUBLIC_PATHS: /terms
...
- **وضعیت:** ✅ تأیید شده
- **زمینه:**
  - CompanionPanel در bare mode اسکرول‌بار مخفی دارد؛ کاربر نمی‌دانست محتوا ادامه دارد.
  - کاربران ممکن است در چتِ همدم و کامنتِ بلاگ با کیبوردِ لاتین بنویسند.
- **تصمیم:**
  - **CompanionPanel:** `useRef` + `ResizeObserver` + `onScroll` — وقتی `scrollHeight > clientHeight + 4`، fade gradient با `rgba(var(--rgb-card), 0.9)` در انتهای پنل نمایش داده می‌شود؛ با اسکرول ناپدید می‌شود.
  - **ChatWindow + CommentForm:** تشخیصِ `/[a-zA-Z]{2,}/` در onChange → پیامِ راهنمای «لطفاً کیبورد را به فارسی تغییر دهید» (غیربلاک‌کننده). در ChatWindow: focus textarea بعد از ارسال هم اصلاح شد.
