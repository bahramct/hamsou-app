...
| ۵ | موبایل‌سازی + PWA | فیچر | 🟠 High | 🔄 در جریان | — | روز ۲~ | PWA بازار ایران موبایل‌محور است. **فاز A (DECISION-116):** هاردنینگِ ریسپانسیوِ داشبورد — clamp سیّال + نردبانِ نقطه‌شکست + ضدِسرریز، اسکوپ به .dsh-؛ ماکاپِ مرجع `mockups/app-mobile-dashboard.html`. مرجعِ زیبایی = همیار (warm theme = Warm Paper). **نوارِ تبِ پایین:** DECISION-117 سپس **DECISION-119** (یکپارچهٔ ۵‌تبه: مسیر/گزارش/[خانه برجسته]/همیار«به‌زودی»/پروفایل — سبکِ همیار). **فاز B (DECISION-118):** ریسپانسیوِ goal/profile + clearanceِ تب + بالا‌بردنِ FABها. **DECISION-120:** رفعِ سرریزِ goal (grid blowout→minmax(0,1fr)) + حذفِ نشانگرِ dev نکست + مخفیِ ابزارهای dev موبایل. **DECISION-121 (نصب‌پذیریِ PWA):** manifest + آیکون‌ها + متادیتای iOS + service worker (prod-only) + /offline. **A1 (DECISION-122):** ۶ مودال max-h+اسکرول. **اصلاحاتِ موبایل (DECISION-123/124/125):** پاپ‌اوورِ استوری press-hold (نگه‌داشتن نمایش، رها پنهان) + چت = حبابِ کارتیِ iOS + **FAB = یک پلاسِ شیشه‌ایِ گسترش‌یابنده** (Liquid Glass؛ پرو→همدم/پشتیبانی، رایگان→مستقیم همدم). **DECISION-126:** رفعِ ریشه‌ایِ کلیپِ حبابِ تایم‌لاین (حذفِ overflow-x القاکنندهٔ overflow-y) + شیشه‌ایِ واقعی FAB (زیرش پیدا) + رنگِ بژِ پشتیبانی. **DECISION-128:** سیستمِ سه‌تم (لایت/دارک/ایندیگو از پالتِ همیار، حذفِ «سیستم»، ایندیگو فقط در اپ). **DECISION-129:** اعمالِ عمیقِ ایندیگو — رفعِ «شیشهٔ سفید روی زمینهٔ تیره» (overrideِ اسکوپ‌شدهٔ `[data-theme="indigo"]` برای glass/blob/دکمه/داشبورد). **DECISION-130 (بازپایه‌گذاریِ کامل روی همیار — منبعِ توکنِ واحد):** ملاکِ مطلق = همیار؛ کلِ پروژه از یک منبع استایل می‌گیرد. استخراجِ خط‌به‌خطِ همیار → `docs/hamyar-design-system.md`. لایهٔ توکنِ معناییِ همیار (surface/accent/feedback/shadow-glow/ambient) در `globals.css` per-theme با مقادیرِ دقیق + الیاسِ توکن‌های لگاسی. **فاز۱ لایت پایه انجام شد** (glass/blob/btn-primary token-driven)؛ فاز۲ دارک، فاز۳ ایندیگو + مهاجرتِ کامپوننتیِ dsh-/jp-/pf- و افزودنِ افکت‌ها ادامه دارد. ماکاپِ ۳تم `mockups/hamyar-dna-dashboard.html`. **سپس → epic همیار (سندِ معماری اول)** |
...
| ۲۷ | بازطراحی قالب‌های ایمیل + رفعِ اسپم | اصلاح | 🔴 Critical | ✅ تمام | — | — | DECISION-084 — قالبِ فعال‌سازی حساب اختصاصی + text/plain برای همه + دیزاینِ RTL همسو + حذفِ دکمهٔ DEV ایمیل — ۲۰۲۶-۰۶-۱۳ |
| ۲۸ | بازطراحیِ تایم‌لاینِ JourneyRail | UI/UX ✨ | 🟠 High | ✅ تمام | — | ساعت ۳~ | DECISION-105 — پیکان‌های زنجیره‌ای RTL + حبابِ استوری + نشانِ همراه بالا؛ بدونِ اثرِ مخرب روی GoalTile — ۲۰۲۶-۰۶-۱۷ |
| ۲۹ | منوی پلن‌ها + قوانین + صفحهٔ عمومی | UI ✨ | 🟠 High | ✅ تمام | — | ساعت ۳~ | DECISION-115 — «پلن‌ها» به منوی عمومی، /terms مستقل، فوتر بازطراحی، /plans دوگانه (عمومی/لاگین)، PricingPublicView (کارت+جدول+FAQ) — ۲۰۲۶-۰۶-۲۳ |
...
- **منبع:** درخواست مالک — گروه‌بندی آیتم‌های ناوبری + مخفی‌کردن اسکرول‌بار
- **تغییرات:**
  - [x] ساختار `NAV_GROUPS` جایگزین `NAV_ITEMS` تخت شد — ۶ گروه با عناوین ثابت
  - [x] گروه‌ها: داشبورد / کاربران / پشتیبانی / مدیریت محتوا / زیر ساخت ها / تنظیمات سایت
  - [x] عنوانِ ناوِ «تنظیمات سایت» → «تنظیمات راهنما» (پوشهٔ `/admin/settings`)
  - [x] عنوانِ «پیامک» → «سرویس پیامک»، «ایمیل» → «سرویس ایمیل»، «پرداخت» → «درگاه پرداخت»
  - [x] عنوانِ «چت آنلاین» → «ارتباط آنلاین»، «پیام‌های تماس» → «فرم تماس با ما»
  - [x] عرضِ ساید بار از `w-60` (240px) به `w-64` (256px) برای جا دادن برچسب‌های بلندتر
  - [x] کلاسِ `.scrollbar-none` به globals.css اضافه شد + روی پنلِ اسکرول‌شوندهٔ ساید بار اعمال
  - [x] منوی موبایل: اسکرول‌بار مخفی + سقفِ `max-h-[80dvh]` برای نمایشِ صحیح
  - [x] `tsc --noEmit` بدون خطا
...
*آخرین بروزرسانی: ۲۰۲۶-۰۶-۱۸ — اصلاحاتِ جامعِ UI: JourneyRail، داشبورد (ساعت→بِنتو)، CompanionPanel، UserActions+API، هشدارِ فارسی*
